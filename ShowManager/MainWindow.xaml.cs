...
namespace ShowManager
{
  ...
  public partial class MainWindow : Window, ICommandCatcher
  {
    ...
    // Окно базы исполнителей
    public bool isBaseArtistsActive = false;
    BaseArtists wndBaseArtists;
    ...
    // Исполнители
    public void Menu_Catalogue_Artists(object sender, RoutedEventArgs e)
    {
      if (!isBaseArtistsActive)
      {
        wndBaseArtists = new BaseArtists(this, wndDragDrop, gentres);
        wndBaseArtists.Show();
        isBaseArtistsActive = true;
      }
      else
      {
        if (wndBaseArtists.WindowState == WindowState.Minimized) wndBaseArtists.WindowState = WindowState.Normal;
      }
    }
    ...
    {
      var wndGentres = new BaseGentres(wndDragDrop, gentres);
